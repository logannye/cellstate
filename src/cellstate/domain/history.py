...
class CellHistory(SchemaModel):
    ...
    def consistent_subject_and_ids(self) -> CellHistory:
        ...
        mismatched = [
            event.event_id for event in self.events if event.subject_id != self.subject_id
        ]
        if mismatched:
            raise ValueError(
        ...
        for event in self.events:
            if isinstance(event, InterventionEvent) and event.actual_perturbation is not None:
                ...
                if event.actual_perturbation.status in {
                    PerturbationStatus.MEASURED,
                    PerturbationStatus.FAILED,
                }:
                    premature_evidence = [
                        evidence_id
                        for evidence_id in event.actual_perturbation.evidence_event_ids
                    ...
                    if premature_evidence:
                        raise ValueError(
                            "measured or failed perturbation evidence cannot predate the "
                            f"intervention: {sorted(premature_evidence)}"
                        )
        return self
    ...
    def through(self, time_seconds: float) -> tuple[CellEvent, ...]:
        return tuple(event for event in self.events if event.time_seconds <= time_seconds)

    def between(self, start: float, end: float) -> tuple[CellEvent, ...]:
        return tuple(event for event in self.events if start < event.time_seconds <= end)
