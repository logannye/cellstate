...
class StagedTrainingInventory(SchemaModel):
    ...
    entries: tuple[StagedTrainingEntry, ...] = Field(min_length=1)
...
class ContainedExecutionObservation(SchemaModel):
    ...
    def outcome_is_exact(self) -> Self:
        ...
        if self.outcome != expected or (self.timed_out and self.oom_killed):
...
class ContainedTrainingWorkerObservation(SchemaModel):
    ...
    source_closed_before_post_verification: Literal[True] = True
...
class ContainedTrainingObservation(SchemaModel):
    ...
    def success_and_identity_are_exact(self) -> Self:
        ...
        if execution.outcome != "success" or execution.exit_code != 0:
...
class DockerExecutor:
    ...
    def _owned_directory(self, path: Path, *, token_payload: bytes) -> None:
        ...
        if created or not token.exists():
            if tuple(path.iterdir()) and not created:
                raise ContainedExecutionError("unowned execution staging directory is not empty")
            flags = (
                os.O_WRONLY
                | os.O_CREAT
                | os.O_EXCL
            ...
            try:
                descriptor = os.open(token, flags, 0o400)
                try:
                    if os.write(descriptor, token_payload) != len(token_payload):
    ...
    def _prepare_output_stage(self, *, execution_id: str, output_path: Path) -> None:
        ...
        try:
            execution_state = execution_root.lstat()
        except FileNotFoundError:
            pass
        else:
            if not stat.S_ISDIR(execution_state.st_mode):
                raise ContainedExecutionError("execution stage root must not be a symlink")
            directories = [execution_root]
            for current, names, _ in os.walk(execution_root, followlinks=False):
                current_path = Path(current)
                for name in names:
                    child = current_path / name
                    ...
                    if stat.S_ISDIR(child_state.st_mode):
                        ...
                        raise ContainedExecutionError("execution stage contains a symlink")
    ...
    def _run_contained(
        ...
        *,
        ...
        deadline = float(self._monotonic()) + self.policy.wall_clock_seconds
